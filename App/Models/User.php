...
// $valida_nome_banco = verificacao(self::$tbl_banco, "nome_banco", $dados["nome_banco"], "id_banco", $dados["id_banco"], true);
      if($email_check == 0) {
        ...
        $update_stmt->bindValue(":user_name", $data["user_name"]);
        $update_stmt->bindValue(":user_email", $data["user_email"]);
        $update_stmt->bindValue(":user_ddd", $user_ddd);
        $update_stmt->bindValue(":user_number", $user_number);
        $update_stmt->bindValue(":user_password", $data["user_password"]);
        $update_stmt->bindValue(":user_id", $data["user_id"]);
        ...
        throw new \Exception("Ocorreu um erro durante a edição. O email em questão já está em uso.");
      }
    ...
    public function delete($data) {
      ...
      $delete_command = "DELETE FROM ".self::$user_table." WHERE user_id = :user_id";

      $delete_stmt = $pdo_connection->prepare($delete_command);

      $delete_stmt->bindValue(":user_id", $data["user_id"]);
